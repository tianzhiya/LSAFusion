...
class LightLossFunction(nn.Module):
    def __init__(self):
        super(LightLossFunction, self).__init__()
...
class ColorAngleLossModule(nn.Module):
    def __init__(self):
        super(ColorAngleLossModule, self).__init__()

    def forward(self, If, vi_3):
        # Define a function to calculate the angle between two complex numbers
        def angle(complex1, complex2):
            complex1 = torch.complex(complex1, torch.zeros_like(complex1))  # Convert to complex
            complex2 = torch.complex(complex2, torch.zeros_like(complex2))  # Convert to complex
            phase1 = torch.angle(complex1)
            phase2 = torch.angle(complex2)
            return torch.abs(phase1 - phase2)

        # Calculate the color angle loss
        color_angle_loss = torch.mean(
            angle(If[:, :, :, 0], vi_3[:, :, :, 0]) +
            angle(If[:, :, :, 1], vi_3[:, :, :, 1]) +
            angle(If[:, :, :, 2], vi_3[:, :, :, 2])
        )
